...
ZombiePedSkins = {307,308,309,310,311,312,278} --ALTERNATE SKIN LISTS FOR ZOMBIES (SHORTER LIST IS TEXTURED ZOMBIES ONLY)
...
function outbreak(startedResource)
	...
	if startedResource == getThisResource() then
		call(getResourceFromName("scoreboard"), "scoreboardAddColumn", "ZK",_,50) --ADDS TO SCOREBOARD
...
function deanimated( ammo, attacker, weapon, bodypart )
	if (attacker) then
		if (getElementType ( attacker ) == "player") and (getElementType ( source ) == "ped") then
			if (getElementData (source, "zombie") == true) then
				local oldZcount = getElementData ( attacker, "ZK" )
				if oldZcount ~= false then
					setElementData ( attacker, "ZK", oldZcount+1  )
					triggerEvent ( "onZombieWasted", source, attacker, weapon, bodypart )
				else
					setElementData ( attacker, "ZK", 1  )
					triggerEvent ( "onZombieWasted", source, attacker, weapon, bodypart )
				end
			end
		end
