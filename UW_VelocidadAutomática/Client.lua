...
function cc()
	if (not isElement(myveh)) then
		...
		ccEnabled=false
		outputChatBox("#ffffffVelocidad automática #ff0000Desactivada", 255,255,255,true)
		return false
	end
				...
				if targetSpeed > 4 then
					if (limit) then
						if in_array(getVehicleType(veh), allowedTypes) then
							...
							outputChatBox("#ffffffVelocidad automática #00ff00Activada #ffffffa #ffff00"..targetSpeed.. "km/h", 255,255,255,true)
							addEventHandler("onClientRender", getRootElement(), cc)
							ccEnabled=true				
						end
					else
						targetSpeed = round2(targetSpeed)
						outputChatBox("#ffffffVelocidad automática #00ff00Activada#ffffff #ffffffa #ffff00"..targetSpeed.. "km/h", 255,255,255,true)
						addEventHandler("onClientRender", getRootElement(), cc)
						ccEnabled=true
					end
				end
