...
Window_VS = guiCreateWindow(373, 219, 557, 303,"Panel de autos",false)
...
guiGridListAddColumn(Grid_VS, "Nombre", 0.4)
guiGridListAddColumn(Grid_VS, "Precio", 0.3)
...
Button_VS_sn = guiCreateLabel(381, 28, 166, 25, "Spawnear", false, Window_VS)
...
Button_VS_dy = guiCreateLabel(381, 63, 166, 25, "Guardar", false, Window_VS)
...
Button_VS_Warp = guiCreateLabel(381, 98, 166, 25, "Warpear auto", false, Window_VS)
...
Button_VS_Fix = guiCreateLabel(381, 133, 166, 25, "Reparar vehículo", false, Window_VS)
...
Button_VS_lk = guiCreateLabel(381, 168, 166, 25, "Bloquear/Desbloquear", false, Window_VS)
...
Button_VS_sl = guiCreateLabel(381, 203, 166, 25, "Vender vehículo", false, Window_VS)
...
Button_VS_give = guiCreateLabel(381, 238, 166, 25, "Vender a jugador", false, Window_VS)
...
adLabel = guiCreateLabel(0, 269, 296, 24,"Warpear auto: $20000          Reparar auto: $5000", false, Window_VS)  -------------- Nome do seu servidor
...
Window_CHK = guiCreateWindow(screX/2-155,screY/2-60,310,120,"Vender auto",false)
...
Button_CHK_Y = guiCreateButton(17,73,129,36,"Vender",false,Window_CHK)
...
bindKey("F7", "down",
function()
if getElementInterior(localPlayer) == 0 and getElementDimension(localPlayer) == 1 then
...
function WINDOW_CLICK_VEHICLE (button, state, absoluteX, absoluteY)
	...
	elseif (source == Button_VS_sl) then 
		...
		guiSetText(Label_CHK, 'Deseas vender tu "'..carName..'" por $'..carprice-100000)
	...
	elseif source == Button_PLS_Y then
		...
		if row and row ~= -1 then
			...
			if (tonumber(guiGetText (edit_PLS_price)) or 0) >= 0 then
				...
				outputChatBox ( "#c1c1c1Espera una respuesta del jugador.", 10, 250, 10,true )
...
function createPlayersList (row_id)
	showCursor ( true )
	Window_PLS = guiCreateWindow(screX/2-155,screY/2-220,310,420,"Venta de autos",false)
	guiSetVisible(Window_PLS, true)
	guiSetProperty(Window_PLS, "AlwaysOnTop", "true")
	guiWindowSetSizable(Window_PLS, false)
	Label_PLS_info = guiCreateLabel(21,28,266,36,"Ingrese el precio del auto:",false,Window_PLS)
	...
	Button_PLS_N = guiCreateButton(161,379,129,36,"Cancelar",false,Window_PLS)
	...
	local column = guiGridListAddColumn( playerList_PLS, "Jugadores", 0.85 )
	if ( column ) then
...
function(player, acc, price, veh_name, veh_id)
	if player and price and acc and veh_name and veh_id then
		if getPlayerFromName ( player ) then
			...
			outputChatBox ( "#c1c1c1Jogador não pode ser encontrado", 250, 10, 10,true)
		end
	end
end)
...
function createAcceptBuyCarWindow(player,acc,price, veh_name, veh_id)
	...
	Window_ABC = guiCreateWindow(screX/2-155,screY/2-220,410,100,"Venta de autos",false)
	guiSetVisible(Window_ABC, true)
	guiSetProperty(Window_ABC, "AlwaysOnTop", "true")
	guiWindowSetSizable(Window_ABC, false)
	Label_ABC_info = guiCreateLabel(10,28,390,36,player.." te ofreció un "..veh_name.." por "..price.."$ (Tu dinero actual: "..getPlayerMoney() .."$)",false,Window_ABC)
...
carColumn = guiGridListAddColumn(carGrid, "Nombre", 0.5)
costColumn = guiGridListAddColumn(carGrid, "Precio", 0.4)
...
for i, M in ipairs(ShopTable) do
	ShopMarker = createMarker(M["PosX"], M["PosY"], M["PosZ"], "cylinder", 0.8, 38, 122, 216)
	ShopMarkerShader = createMarker(M["PosX"], M["PosY"], M["PosZ"], "cylinder", 0.8, 38, 122, 216)
	setElementDimension ( ShopMarker, 1 )
	setElementDimension ( ShopMarkerShader, 1 )
...
function()
	...
	elseif (source == carButton) then
		if guiGridListGetSelectedItem(carGrid) then
			...
			outputChatBox("#c1c1c1Você acabou de comprar um: #00FF66"..carName, 38, 122, 216, true)
			guiSetVisible(VehicleShop_Window, false)
			showCursor(false)
		...
		if(getElementData(car,'ownercar')) then
			...
			if(sx) and (sy) then
				...
				if(distance < 20) then
					...
					if(owner) then
						dxDrawBorderedCarText("Dueño: "..(owner), sx, sy, sx, sy, tocolor(255, 255, 255, 255), fontbig, "default-bold", "center")	
					end
				end
			end
		end
