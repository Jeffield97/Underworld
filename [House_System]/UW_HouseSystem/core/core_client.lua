...
addEventHandler( 'onClientResourceStart', resourceRoot, function()
  ...
  --Entrance--
  hc_gui['label_ec'] = guiCreateLabel( 0.042, 0.05, 0.4, 0.1, 'Coordenada marker entrada a la casa', true, hc_gui['main'] );
  ...
  hc_gui['btn_ecPick'] = guiCreateButton( 0.063, 0.36, 0.39, 0.06, 'Obtener coordenadas', true, hc_gui['main'] ); --1
  
  --Entrance teleport--
  hc_gui['label_etc'] = guiCreateLabel( 0.009, 0.45, 0.5, 0.1, 'Coordenadas de salida dentro de la casa', true, hc_gui['main'] );
  ...
  hc_gui['btn_etcPick'] = guiCreateButton( 0.063, 0.76, 0.39, 0.06, 'Obtener coordenadas', true, hc_gui['main'] ); --2
  
  --Exit--
  hc_gui['label_exc'] = guiCreateLabel( 0.55, 0.05, 0.4, 0.1, 'Coordenada marker salida de la casa', true, hc_gui['main'] );
  ...
  hc_gui['btn_excPick'] = guiCreateButton( 0.55, 0.36, 0.39, 0.06, 'Obtener coordenadas', true, hc_gui['main'] ); --3
  
  --Exit teleport--
  hc_gui['label_extc'] = guiCreateLabel( 0.51, 0.45, 0.47, 0.1, 'Coordenadas de salida fuera de la casa', true, hc_gui['main'] );
  ...
  hc_gui['btn_extcPick'] = guiCreateButton( 0.55, 0.76, 0.39, 0.06, 'Obtener coordenadas', true, hc_gui['main'] ); --4
  ...
  hc_gui['btn_reset'] = guiCreateButton( 0.88-0.21, 0.915, 0.2, 0.05, 'Resetear', true, hc_gui['main'] );
  hc_gui['btn_create'] = guiCreateButton( 0.0, 0.915, 0.3, 0.05, 'Crear Casa', true, hc_gui['main'] );
  ...
  addEventHandler( 'onClientGUIClick', hc_gui['btn_create'], function( btn )
    ...
    for i, v in pairs( hc_edit ) do
      if guiGetText( v ) == '' then
        outputChatBox( '* Todos los campos de edición deben completarse!', 255, 51, 36 );
        return false;
      end;
    end;
    for i, v in pairs( hc_gui ) do
  ...
  hm_gui['main'] = guiCreateWindow( scX-360, scY-360, 350, 350, 'Sistema de casas', false );
  ...
  hm_gui['label_key'] = guiCreateLabel( 0.7, 0.08, 0.255, 0.1, 'Establecer clave', true, hm_gui['main'] );
  ...
  hm_gui['label_enter'] = guiCreateLabel( 0.03, 0.45, 0.265, 0.1, 'Entrar a casa', true, hm_gui['main'] );
  ...
  hm_gui['label_newowner'] = guiCreateLabel( 0.365, 0.45, 0.255, 0.1, 'Establecer nuevo dueño', true, hm_gui['main'] );
  ...
  hm_gui['label_destroy'] = guiCreateLabel( 0.68, 0.45, 0.295, 0.1, 'Eliminar esta casa', true, hm_gui['main'] );
  ...
  hm_gui['label_owner'] = guiCreateLabel( 0.05, 0.88, 0.25, 0.1, 'Dueño: ', true, hm_gui['main'] );
  hm_gui['label_price'] = guiCreateLabel( 0.4, 0.82, 0.25, 0.1, 'Precio $: ', true, hm_gui['main'] );
  ...
  addEventHandler( 'onClientGUIClick', root, function( btn )
    ...
    elseif source == hm_gui['icon_key'] then
      ...
      guiSetText( hm_gui['onChange_edit_Old'], 'Clave actual' );
      guiSetText( hm_gui['onChange_edit_New'], 'Clave nueva' );
      guiSetText( hm_gui['onChange_edit_RepeatNew'], 'Repite clave nueva' );
      playSound( 'sounds/click_icon.wav' );
      guiSetInputEnabled( true );
    elseif source == hm_gui['icon_newowner'] then
      guiSetVisible( hm_gui['main'], false );
      guiSetVisible( hm_gui['wnd_key_onNewOwner'], true );
      guiSetText( hm_gui['onNewOwner_keyEdit'], 'Clave' );
      guiSetText( hm_gui['onNewOwner_nameEdit'], 'Cuenta del nuevo dueño' );
      playSound( 'sounds/click_icon.wav' );
      guiSetInputEnabled( true );
    ...
    if source == hm_gui['onBuy_confirm'] then
      ...
      if #passTxt >= getElementData( me, 'k_len' ) then
        if not passTxt:find( ' ' ) then
          ...
          outputChatBox( '* La clave no debe contener espacio!', 255, 51, 36 );
        end;
      else
        outputChatBox( '* La clave debe tener al menos '..tostring( getElementData( me, 'k_len' ) )..' caracteres!', 255, 51, 36 );
      end;
  ...
  addEventHandler( 'onClientGUIClick', root, function( btn )
    ...
    if source == hm_gui['onSale_confirm'] then
      ...
      if guiGetText( hm_gui['onSale_edit'] ) == getElementData( mrk, 'HS_INFO' )[8] then
        ...
        outputChatBox( '* Acabas de vender tu casa!', 255, 255, 0 );
        guiSetText( hm_gui['onSale_edit'], '' );
        guiSetVisible( hm_gui['wnd_key_onSale'], false );
        playSound( 'sounds/click_confirm.wav' );
        guiSetInputEnabled( false );
      else
        outputChatBox( '* Clave incorrecta!', 255, 51, 36 );
      end;
  ...
  hm_gui['onChange_label'] = guiCreateLabel( 0.05, 0.15, 0.9, 0.4, "Para cambiar su clave, escribe tu clave ACTUAL y luego una NUEVA.", true, hm_gui['wnd_key_onChange'] ); 
  guiLabelSetHorizontalAlign( hm_gui['onChange_label'], 'center', true );
  guiLabelSetColor( hm_gui['onChange_label'], 0, 153, 255 );
  hm_gui['onChange_edit_Old'] = guiCreateEdit( 0.14, 0.3, 0.7, 0.1, 'Clave actual', true, hm_gui['wnd_key_onChange'] );
  hm_gui['onChange_edit_New'] = guiCreateEdit( 0.14, 0.45, 0.7, 0.1, 'Clave nueva', true, hm_gui['wnd_key_onChange'] );
  hm_gui['onChange_edit_RepeatNew'] = guiCreateEdit( 0.14, 0.6, 0.7, 0.1, 'Repite clave nueva', true, hm_gui['wnd_key_onChange'] );
  hm_gui['onChange_confirm'] = guiCreateButton( 0.2, 0.75, 0.23, 0.11, 'Confirmar', true, hm_gui['wnd_key_onChange'] );
  hm_gui['onChange_cancel'] = guiCreateButton( 0.525, 0.75, 0.23, 0.11, 'Cancelar', true, hm_gui['wnd_key_onChange'] );
      ...
      if guiGetText( hm_gui['onChange_edit_Old'] ) == currKey then
        if #guiGetText( hm_gui['onChange_edit_New'] ) >= getElementData( me, 'k_len' ) then
          if guiGetText( hm_gui['onChange_edit_New'] ) == guiGetText( hm_gui['onChange_edit_RepeatNew'] ) then
            ...
            outputChatBox( '* Nueva clave ha sido establecida como #00FF00'..guiGetText( hm_gui['onChange_edit_RepeatNew'] ), 255, 51, 36, true );
            ...
            outputChatBox( '* Las nuevas a establecer claves no coinciden, verifica que sean iguales!', 255, 51, 36 );
          end;
        else
          outputChatBox( '* La clave debe contener al menos '..tostring( getElementData( me, 'k_len' ) )..' caracteres!', 255, 51, 36 );
        end;
      else
        outputChatBox( '* La clave actual no es correcta!', 255, 51, 36 );
      end;
  ...
  hm_gui['onNewOwner_label'] = guiCreateLabel( 0.05, 0.11, 0.9, 0.4, "Escriba el nombre de la cuenta del nuevo dueño y su clave ACTUAL. NOTA: ¡después de esta operación ya no serás el dueño de esta casa!", true, hm_gui['wnd_key_onNewOwner'] ); 
  guiLabelSetHorizontalAlign( hm_gui['onNewOwner_label'], 'center', true );
  guiLabelSetColor( hm_gui['onNewOwner_label'], 0, 153, 255 );
  hm_gui['onNewOwner_keyEdit'] = guiCreateEdit( 0.14, 0.45, 0.7, 0.1, 'Clave', true, hm_gui['wnd_key_onNewOwner'] );
  hm_gui['onNewOwner_nameEdit'] = guiCreateEdit( 0.14, 0.6, 0.7, 0.1, 'Cuenta nuevo dueño', true, hm_gui['wnd_key_onNewOwner'] );
        ...
        if guiGetText( hm_gui['onNewOwner_nameEdit'] ) ~= getPlayerName( me ) then
          if getPlayerByAccountName( guiGetText( hm_gui['onNewOwner_nameEdit'] ) ) then
            if guiGetText( hm_gui['onNewOwner_keyEdit'] ) == getElementData( getHouseByID( getElementData( me, 'mrk_in' ) ), 'HS_INFO' )[8] then
              ...
              outputChatBox( '* El nuevo dueño de esta casa es #00FF00'..guiGetText( hm_gui['onNewOwner_nameEdit'] ):gsub( "%x%x%x%x%x%x", "" ), 255, 51, 36, true );
              guiSetText( hm_gui['onNewOwner_nameEdit'], '' );
              guiSetVisible( hm_gui['wnd_key_onNewOwner'], false );
              playSound( 'sounds/click_confirm.wav' );
              guiSetInputEnabled( false );
            else
              outputChatBox( '* Clave incorrecta, intentalo nuevamente.', 255, 51, 36 );
              guiSetText( hm_gui['onNewOwner_nameEdit'], '' );
              guiSetText( hm_gui['onNewOwner_keyEdit'], '' );
            end;
          else
            outputChatBox( '* No se encontró el jugador!', 255, 51, 36 );
          end;
        end;
      else
        outputChatBox( "* El nombre del dueño no debe contener espacios!", 255, 51, 36 );
      end;
