...
function requestRespray() 
	if( getPlayerMoney( source ) >= paynsprayCosts ) then
		...
		setTimer( function (s,v) 
			...
			outputChatBox( "Tu vehículo ha sido " .. action .. " por un costo de $" .. paynsprayCosts .. ".", s, 0, 255, 0 )
			triggerEvent( "onPayNSprayFix", getRootElement(), v, s )
			
			end, 1000, 1, source, vehicle )
	else
		outputChatBox( "No tienes suficiente dinero! El precio es $" .. paynsprayCosts .. ".", source, 255, 255, 0 )
	end
end
