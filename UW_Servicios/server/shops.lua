	...
	if( articleID == "JETPACK" ) then
		if( getElementData( player, "jetpackFuel" ) ) then
			if( tonumber( getElementData( player, "jetpackFuel" ) ) == tonumber( get( "MaxJetpackFuel" ) ) ) then
				outputChatBox( "No puedes poner más combustible al jetpack!", player, 255, 255, 0 )
				cancelEvent()
			end
		end
	end
...
function buyShopArticle( shopID, articleID )
	...
	if( getPlayerMoney( source ) < shops[shopID].Articles[articleID].Price ) then
		outputChatBox("No tienes dinero suficiente para " .. shops[shopID].Articles[articleID].Name .. "!", source, 255, 255, 0 )
		return
	end
	...
	-- give it
	outputChatBox("Compraste " .. shops[shopID].Articles[articleID].Name .. "! Costo: $" .. shops[shopID].Articles[articleID].Price, source, 0, 255, 0)
